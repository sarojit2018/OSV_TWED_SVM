...
targets = zeros(2,10*51);
outputs = zeros(2,10*51);
cnt_loop = 1;

scores = zeros(1,100*50);
...
%setglobal(thres);
svmmodel = fitcsvm(trainX,trainY,'KernelFunction','linear','Standardize',true,'verbose',1);
...
testXg = X(1:25,:);%check for genuine
testYg = Y(1:25,:);%check for genuine
...
for i=1:26
    targets(2,cnt_loop) = 1;
    if(scoref_single_vect(i)>0)
       outputs(2,cnt_loop) = min(1 - 0.1*rand,scoref_single_vect(i));
       outputs(1,cnt_loop) = 1 - outputs(2,cnt_loop);
    else
        outputs(2,cnt_loop) = max(0.1*rand,1 + scoref_single_vect(i));
        outputs(1,cnt_loop) = 1 - outputs(2,cnt_loop);
        
    end
    %outputs(1,cnt_loop) = predictYf(i);
    scores(1,cnt_loop) = scoref_single_vect(i);
...
for i=1:25
    targets(1,cnt_loop) = 1;
    if(scoreg_single_vect(i)>0)
       outputs(1,cnt_loop) = min(1-0.1*rand,scoreg_single_vect(i));
       outputs(2,cnt_loop) = 1 - outputs(1,cnt_loop);
    else
        outputs(1,cnt_loop) = max(0.1*rand,1 + scoreg_single_vect(i));
        outputs(2,cnt_loop) = 1 - outputs(1,cnt_loop);
    end
    %outputs(1,cnt_loop) = predictYg(i);
    scores(1,cnt_loop) = scoreg_single_vect(i);
    cnt_loop = cnt_loop + 1;
