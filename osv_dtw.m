...
targets = zeros(2,20*40);
outputs = zeros(2,20*40);
...
for i=1:25
  ...
  for j=1:num_feat%columns(features)
    ...
    gen_tr{u_num,i} = [gen_mat_all(:,1:4) gen_AR_params(:,1:4) tstamp];
...
for i=1:25
  ...
  for j=1:num_feat%columns(features)
    ...
    %disp(size(tstamp));
    forg_tr{u_num,i} = [forg_mat_all(:,1:4) forg_AR_params(:,1:4) tstamp];
...
for i=6:25
    ...
    if(avg_dtw_dist <= average_threshold)
        ...
        outputs(1,cnt_loop) = max(0.6,avg_dtw_dist/average_threshold);
        ...
        testY(i-5,1) = -1;
        targets(1,cnt_loop) = 1;
        outputs(2,cnt_loop) = max(0.6,average_threshold/avg_dtw_dist);
