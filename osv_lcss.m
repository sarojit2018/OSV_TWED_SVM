...
for i=1:25
  ...
  for j=1:num_feat%columns(features)
    ...
    gen_tr{u_num,i} = [gen_mat_all gen_AR_params tstamp];
...
for i=1:25
  ...
  for j=1:num_feat%columns(features)
    ...
    %disp(size(tstamp));
    forg_tr{u_num,i} = [forg_mat_all forg_AR_params tstamp];
...
for i=6:25
    avg_dtw_dist = 0.0;
    for j=1:5
        avg_dtw_dist = avg_dtw_dist + lcss(gen_tr{u_num,i},gen_tr{u_num,j});
    end
...
for i=1:25
    avg_dtw_dist = 0.0;
    for j=1:5
        avg_dtw_dist = avg_dtw_dist + lcss(forg_tr{u_num,i},gen_tr{u_num,j});
    end
    avg_dtw_dist = avg_dtw_dist/5;
