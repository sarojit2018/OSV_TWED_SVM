...
alphas = 0:0.5:6;
[~,loop_count] = size(alphas);

eers = zeros(loop_count,1);

for loop_var = 1:loop_count
    alpha_cur = alphas(loop_var);
    setglobalsize(alpha_cur);
    FAR = 0;
    FRR = 0;
...
for i=1:25
  ...
  for j=1:num_feat%columns(features)
    ...
    gen_tr{u_num,i} = [gen_AR_params(:,1:5) tstamp];
...
for i=1:25
  ...
  for j=1:num_feat%columns(features)
    ...
    %disp(size(tstamp));
    forg_tr{u_num,i} = [forg_AR_params(:,1:5) tstamp];
...
testXf = X(31:50,:);%check for forgery 
testYf = Y(31:50,:);%check for forgery

testXg = X(6:25,:);%check for genuine
testYg = Y(6:25,:);%check for genuine
